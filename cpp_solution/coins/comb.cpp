...
struct combinations {
  ...
  void putCombination() {
    // Put the combinations into the full combination set and reset stack
    all_combinations.push_back(combs);
    // clear the stack
    combs.clear();
  }
  void add_number(int n) { combs.push_back(n); }
  std::vector<std::vector<int>> return_Combinations() {
...
int main() {
  ...
  int target = 5;
  std::cout << "The combination count: " << combinations(coins, target, table)
            << std::endl;
  // print cache table
